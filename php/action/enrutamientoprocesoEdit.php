...
if($generaEtiquetaProduccion == "" || $idFinca == 0 || $idProducto == 0 || $idServicio == 0 || $idMaterialesVegetales == 0 || $idProcesoOrigen == 0 || $idProcesoDestino == 0) {
    header("Location: ../../enrutamientoproceso/enrutamientoproceso_editar.php?id=$id&ge=$generaEtiquetaProduccion&f=$idFinca&p=$idProducto&s=$idServicio&mv=$idMaterialesVegetales&po=$idProcesoOrigen&pd=$idProcesoDestino&tm=$idTipoMovimiento&er1");
    exit;
}

$enrutamientoprocesoDAO = new enrutamientoprocesoDAO();
$enrutamientoproceso = $enrutamientoprocesoDAO->getById($id);
...
$enrutamientoprocesoDAO->update($enrutamientoproceso);
