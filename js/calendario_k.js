jQuery.fn.calendarioDW = function(inicio,fin) {
   ...
   this.each(function(){
		...
		function mostrarCalendario(inicio,fin){
			if(!mostrando){
				mostrando = true;

				//es que hay que mostrar el calendario				//dias de la semana
				...
				muestraDiasMes(mes, ano, inicio,fin);
		...
		function muestraDiasMes(mes, ano, inicio,fin){
			...
			for (var i=0; i<7; i++){
				...
				var t_mes=mes+1;
				var t_contadorDias = contadorDias-1;
				if (t_mes<10){
					t_mes = "0"+t_mes; 
				}
				if (t_contadorDias<10){
					t_contadorDias="0"+t_contadorDias;
				}
				//alert(inicio);
				if (((ano+"/"+t_mes+"/"+t_contadorDias) < inicio) || ((ano+"/"+t_mes+"/"+t_contadorDias) > fin)){
				codigoDia = codigoDia.replace((contadorDias-1),'');
				diaActual = $(codigoDia);
		...
		function validarFechaEscrita(fecha){
			...
			if (arrayFecha.length!=3)
				return false;
		return checkdate(arrayFecha[2], arrayFecha[1], arrayFecha[0]);
		}
   });
